  ...
  ) : itype(_Array_ptr<VOID>)
{
  ...
  //
  volatile _Array_ptr<UINT8>   Destination8;
  _Array_ptr<CONST UINT8>      Source8;
  volatile _Array_ptr<UINT32>  Destination32;
  _Array_ptr<CONST UINT32>     Source32;
  volatile _Array_ptr<UINT64>  Destination64;
  _Array_ptr<CONST UINT64>     Source64;
  UINTN            Alignment;
  ...
  if ((((UINTN)DestinationBuffer & 0x7) == 0) && (((UINTN)SourceBuffer & 0x7) == 0) && (Length >= 8)) {
    if (SourceBuffer > DestinationBuffer) {
      ...
      Destination8 = Destination64;
      Source8      = Source64;
      while (Length-- != 0) {
        *(Destination8++) = *(Source8++);
      }
    } else if (SourceBuffer < DestinationBuffer) {
      Destination64 = ((UINTN)DestinationBuffer + Length);
      Source64      = ((UINTN)SourceBuffer + Length);
    ...
    if (SourceBuffer > DestinationBuffer) {
      ...
      Destination8 = DestinationBuffer + (Length - 1);
      Source8      = SourceBuffer + (Length - 1);
      while (Length-- != 0) {
        *(Destination8--) = *(Source8--);
      }
    }
